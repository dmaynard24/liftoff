...
function normalizeHex(hex) {
    ...
    if (h.length === 6)
        return '#' + h;
    if (h.length === 8)
        return '#' + h.slice(0, 6); // strip alpha channel
...
function deduplicateGrayscale(tokens) {
    const kept = [];
...
export function extractColors(html) {
    ...
    const raw = (html.match(/#[0-9a-fA-F]{3,8}\b/g) ?? []).filter((h) => h.length === 4 || h.length === 7 || h.length === 9);
    ...
    const grayscaleRaw = deduplicateGrayscale(all.filter((t) => isGrayscale(t.value))).slice(0, MAX_GRAYSCALE);
...
export function extractCSSVars(html) {
    const vars = [];
    const seen = new Set();
...
export function extractTypography(html) {
    const families = unique((html.match(/font-family:\s*([^;"}{]+)/g) ?? []).map((s) => s.replace('font-family:', '').trim()));
    const sizes = unique((html.match(/font-size:\s*([^;"}{]+)/g) ?? []).map((s) => s.replace('font-size:', '').trim()));
    const weights = unique((html.match(/font-weight:\s*([^;"}{]+)/g) ?? []).map((s) => s.replace('font-weight:', '').trim()));
    const lineHeights = unique((html.match(/line-height:\s*([^;"}{]+)/g) ?? []).map((s) => s.replace('line-height:', '').trim()));
    const letterSpacings = unique((html.match(/letter-spacing:\s*([^;"}{]+)/g) ?? []).map((s) => s.replace('letter-spacing:', '').trim()));
...
// ─── Spacing ──────────────────────────────────────────────────────────────────
export function extractSpacing(html) {
...
export function extractRadius(html) {
    return unique((html.match(/border-radius:\s*([^;"}{]+)/g) ?? []).map((s) => s.replace('border-radius:', '').trim()));
}
export function extractShadows(html) {
    return unique((html.match(/box-shadow:\s*([^;"}{]+)/g) ?? []).map((s) => s.replace('box-shadow:', '').trim()));
}
export function extractAnimations(html) {
    const transitions = unique((html.match(/transition:\s*([^;"}{]+)/g) ?? []).map((s) => s.replace('transition:', '').trim()));
    const transforms = unique((html.match(/transform:\s*([^;"}{]+)/g) ?? []).map((s) => s.replace('transform:', '').trim()));
    const animations = unique((html.match(/animation:\s*([^;"}{]+)/g) ?? []).map((s) => s.replace('animation:', '').trim()));
    return { transitions, transforms, animations };
}
export function extractLayout(html) {
    const maxWidths = unique((html.match(/max-width:\s*([^;"}{]+)/g) ?? []).map((s) => s.replace('max-width:', '').trim()));
    const gridColumns = unique((html.match(/grid-template-columns:\s*([^;"}{]+)/g) ?? []).map((s) => s.replace('grid-template-columns:', '').trim()));
    const breakpoints = unique((html.match(/@media[^{]+\{/g) ?? []).map((s) => s.replace('@media', '').replace('{', '').trim()));
    return { maxWidths, gridColumns, breakpoints };
}
// ─── Main ─────────────────────────────────────────────────────────────────────
export function extractAllTokens(html) {
    return {
